function polyout = discshift_old(varargin)
...
% all the N parameters, i.e., |a_i(k) - a_i(k+1)| <= b_i. The resultant
...
if ((nargin < 2) || (nargin > 5))
    ...
    if (nargin == 2)
        targetin = 1;
        bounds = ones(polyin.vertices(targetin),1);
...
for cont=1:N
    A = blkdiag(A,Abar);
    bar{cont} = [kron(ones(eta+1,1),[1;0]); kron(ones(2*eta,1),bounds(cont))];
    b = [b; bar{cont}];
end
